...
class TestNumericTypes:
    ...
    def test_double_step_produces_fractional_values(self, spark):
        ...
        df = _build_single_column(spark, "v", {"type": "double", "range": [0.5, 2.0], "step": 0.01})
    ...
    def test_float_step_distinct_values(self, spark):
        ...
        df = _build_single_column(spark, "v", {"type": "float", "range": [0.0, 1.0], "step": 0.1})
        n_distinct = df.select(F.countDistinct("v")).collect()[0][0]
        assert n_distinct >= 10

    def test_decimal_type(self, spark):
        df = _build_single_column(
            spark, "v", {"type": "decimal", "precision": 8, "scale": 2, "range": [1.0, 100.0]}
        )
...
class TestBooleanType:
    ...
    def test_boolean_explicit_weights(self, spark):
        df = _build_single_column(
            spark, "v", {"type": "boolean", "values": [True, False], "weights": [9, 1]}, rows=10_000
        )
        counts = {r["v"]: r["count"] for r in df.groupBy("v").count().collect()}
        total = sum(counts.values())
...
class TestTemporalTypes:
    ...
    def test_date_within_range(self, spark):
        df = _build_single_column(
            spark, "v", {"type": "date", "begin": "2023-01-01", "end": "2023-12-31"}
        )
        row = df.agg(
            F.min("v").cast("string"),
            F.max("v").cast("string"),
...
class TestUuidAndBinary:
    ...
    def test_binary_type_and_length(self, spark):
        ...
        lengths = df.select(F.length("v").alias("l")).agg(F.min("l"), F.max("l")).collect()[0]
        assert lengths[0] == 16
        assert lengths[1] == 16
...
class TestPercentNulls:
    def test_percent_nulls_applied(self, spark):
        df = _build_single_column(
            spark, "v", {"type": "int", "range": [1, 100], "percent_nulls": 0.2}, rows=10_000
        )
